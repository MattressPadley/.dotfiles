...
if [ "$1" = "cd" ]; then
    ...
    # Default behavior: create/attach to tmux session for selected project
    dir=$(fd . ~/Dev --type d --max-depth 1 --exclude .git --exclude Z_Archive -x echo {/} | sort | \
        fzf --preview 'eza --tree --level 1 --color=always ~/Dev/{} | head -200')
